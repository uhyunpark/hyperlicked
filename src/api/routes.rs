...
use super::types::{
    AccountInfo, ApiState, CancelTriggerOrderRequest, CandleInfo, ChainStatus, FundingInfo,
    MarketInfo, OrderInfo, OrderbookSnapshot, PlaceTriggerOrderRequest, PlaceTriggerOrderResponse,
    PositionInfo, SignedTransaction, SubmitOrderResponse, TriggerOrderInfo,
};
...
pub fn create_router(state: SharedState) -> Router {
    ...
    let api_v1 = Router::new()
        ...
        .route("/accounts/:address/trigger-orders", get(get_trigger_orders))
        // Chain endpoints
        ...
        .route("/trigger-orders", post(place_trigger_order))
        .route("/trigger-orders/:id", axum::routing::delete(cancel_trigger_order_by_id))
        .route("/trigger-orders/cancel", post(cancel_trigger_order))
        // Agent delegation
...
async fn place_trigger_order(
    ...
    let trigger_type = match req.trigger_type.as_str() {
        ...
        _ => return Err((StatusCode::BAD_REQUEST, "Invalid trigger type. Use 'sl' or 'tp'".to_string())),
    };
...
async fn cancel_trigger_order_by_id(
    State(state): State<ApiState>,
    Path(id): Path<String>,
