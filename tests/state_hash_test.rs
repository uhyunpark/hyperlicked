...
fn test_state_hash_determinism() {
    ...
    assert_eq!(hash1, hash2, "Identical operations should produce identical state hashes");
}
...
fn test_deposit_order_independence() {
    ...
    execute_tx(&mut state1, Transaction::Deposit { trader: "alice".into(), amount: 100 });
    execute_tx(&mut state1, Transaction::Deposit { trader: "bob".into(), amount: 200 });

    // State 2: Bob then Alice
    execute_tx(&mut state2, Transaction::Deposit { trader: "bob".into(), amount: 200 });
    execute_tx(&mut state2, Transaction::Deposit { trader: "alice".into(), amount: 100 });

    // Hashes should match (deposits are commutative)
    let hash1 = state1.compute_state_hash();
    let hash2 = state2.compute_state_hash();

    assert_eq!(hash1, hash2, "Independent deposits should produce same state regardless of order");
}
...
fn test_different_operations_different_hash() {
    ...
    execute_tx(&mut state2, Transaction::Deposit { trader: "alice".into(), amount: 200 });
...
fn test_hash_includes_balances() {
    ...
    execute_tx(&mut state, Transaction::Deposit { trader: "alice".into(), amount: 100 });
    let hash_with_deposit = state.compute_state_hash();

    assert_ne!(hash_empty, hash_with_deposit, "Deposit should change state hash");
}
...
fn test_hash_includes_positions() {
    ...
    // Setup: deposits
    execute_tx(&mut state, Transaction::Deposit { trader: "alice".into(), amount: 100_000_000 });
    execute_tx(&mut state, Transaction::Deposit { trader: "bob".into(), amount: 100_000_000 });

    let hash_no_positions = state.compute_state_hash();

    // Bob places ask
    execute_tx(&mut state, Transaction::PlaceOrder {
        trader: "bob".into(),
        symbol: "BTC-USDT".into(),
        side: Side::Ask,
        price: 5_000_000,
    ...
    assert_ne!(hash_no_positions, hash_with_positions, "Positions should change state hash");
}
...
fn test_hash_includes_insurance_fund() {
    ...
    assert_ne!(hash1, hash2, "Insurance fund changes should affect state hash");
}
...
fn test_hash_includes_funding_rates() {
    ...
    assert_ne!(hash1, hash2, "Different funding rates should produce different hashes");
}
...
fn test_hash_includes_trigger_orders() {
    ...
    execute_tx(&mut state1, Transaction::Deposit { trader: "alice".into(), amount: 100_000_000 });
    execute_tx(&mut state2, Transaction::Deposit { trader: "alice".into(), amount: 100_000_000 });
    execute_tx(&mut state1, Transaction::Deposit { trader: "bob".into(), amount: 100_000_000 });
    execute_tx(&mut state2, Transaction::Deposit { trader: "bob".into(), amount: 100_000_000 });

    // Create positions (bob sells, alice buys)
    execute_tx(&mut state1, Transaction::PlaceOrder {
        trader: "bob".into(),
        symbol: "BTC-USDT".into(),
        side: Side::Ask,
        price: 5_000_000,
    ...
    execute_tx(&mut state2, Transaction::PlaceOrder {
        trader: "bob".into(),
        symbol: "BTC-USDT".into(),
        side: Side::Ask,
        price: 5_000_000,
...
fn test_block_execution_deterministic_hash() {
    ...
    assert_eq!(hash1, hash2, "Identical block execution should produce identical state hashes");
}
...
fn test_empty_block_minimal_hash_change() {
    ...
    assert_eq!(account.balance, 100, "Empty block shouldn't change balances");
}
...
fn test_hash_stability() {
    ...
    execute_tx(&mut state, Transaction::Deposit { trader: "alice".into(), amount: 100 });
    execute_tx(&mut state, Transaction::Deposit { trader: "bob".into(), amount: 200 });
