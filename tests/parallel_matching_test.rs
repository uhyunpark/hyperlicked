...
fn test_cross_symbol_same_trader() {
    ...
    state.submit_tx(Transaction::Deposit { trader: "alice".into(), amount: 500_000_000 }).unwrap();
    state.submit_tx(Transaction::Deposit { trader: "bob".into(), amount: 500_000_000 }).unwrap();
    ...
    let hash1 = state.execute(&block1);
    ...
    let hash2 = state.execute(&block2);
    ...
    assert_ne!(alice.position("BTC-USDT").size, 0, "Alice should have BTC position");
    assert_ne!(alice.position("ETH-USDT").size, 0, "Alice should have ETH position");

    // Bob should have opposite positions
    let bob = state.account("bob").unwrap();
    assert_ne!(bob.position("BTC-USDT").size, 0, "Bob should have BTC position");
    assert_ne!(bob.position("ETH-USDT").size, 0, "Bob should have ETH position");
}
